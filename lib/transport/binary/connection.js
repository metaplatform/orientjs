...
Connection.prototype.handleSocketData = function (data) {
  var buffer, result, offset, offset2;
  if (this.remaining) {
 ...
 */
Connection.prototype.process = function (buffer, offset, dblCheck) {
  ...
  if(this.queue.length === 0){
    ...
    if (status === OperationStatus.PUSH_DATA) {
      ...
      return this.process(buffer, offset);
  ...
  if(dblCheck){
    ...
    return this.process(buffer, offset, true);
  }
};
