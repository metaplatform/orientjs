  ...
  return this.exec(command, options)
      ...
      .then(function (response){
        if(response.length > 0){
          ...
          var liveHandle = new EventEmitter();
          liveHandle.token = response[0].token;

          liveHandle.prototype = Object.create(EventEmitter.prototype);

          var wrapperCallback = function(currentToken, operation, result){

            if(currentToken == liveHandle.token) {
              if (operation === 1) {
                liveHandle.emit("live-update", result);
              } else if (operation === 2) {
                liveHandle.emit("live-delete", result);
              } else if (operation === 3) {
                liveHandle.emit("live-insert", result);
                liveHandle.emit("live-create", result);
              }
            }
          };
          ...
          liveHandle.unsubscribe = function(){
            parentDb.server.transport.connection.removeListener("live-query-result", wrapperCallback);
            return parentDb.exec("live unsubscribe " + liveHandle.token).then(function(){
              liveHandle.emit("unsubscribe");
              liveHandle.removeAllListeners();
              console.dir(liveHandle);
              return true;
            });
          };
          ...
          throw new Error("Invalid LIVE query response.");
        }
      });
};
